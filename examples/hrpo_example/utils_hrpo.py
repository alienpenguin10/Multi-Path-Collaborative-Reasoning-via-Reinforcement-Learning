    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # NOTE: HRPO MOD: Validation (Minimal)
            if return_thinking_embeds:
                # Basic validation logic kept for safety, but heavy printing removed
                vocab_size = self.get_input_embeddings().weight.shape[0]
                # Ensure tokens are within valid range (silent check)
                pass
            ...
            # NOTE: HRPO Calculate Thinking State
            # 1. Determine "thinking" status (checking for answer start delimiter, e.g. "####")
            strs = processing_class.batch_decode(input_ids[:, input_len:])
            is_thinking = [self.answer_start not in s for s in strs]

            # 2. Calculate continuous "thinking" state (weighted average of embeddings)
            # Formula: last_thinking_states = W_e^T * p_t / ||p_t||
            last_thinking_states = torch.einsum('bv,vd->bd', probs, self.get_input_embeddings().weight)
            # Safe normalization
            last_thinking_states /= (torch.sqrt((probs ** 2).sum(-1, keepdim=True)) + 1e-8).to(last_thinking_states.dtype)
            
            # NOTE: M3PO Cross-path interaction
            # Apply collaborative reasoning across multiple generation paths
            if enable_cross_path and is_thinking is not None:
                # Store current path states for interaction
                path_embeddings.append(last_thinking_states.clone())
                path_distributions.append(probs.clone())

                # Apply cross-path contextual blending if we have multiple paths
            ...
            # NOTE: HRPO Accumulate Thinking Data
            # Capture hidden states and thinking masks for RL optimization
