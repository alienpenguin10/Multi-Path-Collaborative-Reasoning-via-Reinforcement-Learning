    ...
    if past_key_values is not None:
        ...
        # New since 4.56
            ...
            if hasattr(
                ...
                # Save custom kwargs (like is_m3po_inference) before overwriting
                ...
                try:
                    if needs_device_kw(
                        ...
                        mask_kwargs["device"] = input_ids.device
                except:
                    print(
                ...
                # Restore original kwargs and update with necessary fields
                kwargs = original_kwargs
                kwargs["past_key_values"] = past_key_values
                kwargs["cache_position"] = mask_kwargs["cache_position"]
            else:
...
def LlamaModel_fast_forward(
    ...
    **kwargs,
    ...
    # M3PO/ASLR Logic (Active during generation AND training, disabled only for explicit inference)
    is_m3po_inference = kwargs.get('is_m3po_inference', False)
    #print(f"DEBUG [Unsloth fast_forward]: is_m3po_inference={is_m3po_inference}, training={self.training}, has_m3po={hasattr(self, 'm3po_router')}")
    if not is_m3po_inference and hasattr(self, 'm3po_router'):
        #print(f"DEBUG [Unsloth]: Entering M3PO/ASLR logic (N={self.aslr_num_paths}, T={self.aslr_max_iterations})")
        
        # Router decision: SOFT gating (differentiable)
        router_gate_scores = torch.sigmoid(self.m3po_router.gate(hidden_states))  # (batch_size, seq_len, 1)
        router_weight = router_gate_scores[:, -1, :]  # (batch_size, 1) - weight for last token
        
        # Get last token hidden state
        last_token_hidden = hidden_states[:, -1, :]  # (batch_size, hidden_dim)
        
        # Latent Expansion: Duplicate to N paths
        latent_states = last_token_hidden.unsqueeze(1).repeat(1, self.aslr_num_paths, 1)  # (batch, N, hidden)
        #print(f"DEBUG: Expanded to {self.aslr_num_paths} paths, shape: {latent_states.shape}")
        
        # Latent Iterations: T refinement steps with GUMBEL-SOFTMAX stochasticity
        for t in range(self.aslr_max_iterations):
            # GUMBEL-SOFTMAX: Inject stochastic noise for exploration
            gumbel_noise = F.gumbel_softmax(
                torch.zeros_like(latent_states), 
                tau=self.aslr_gumbel_temp, 
                hard=False
            )
            latent_states = latent_states + gumbel_noise * self.aslr_cross_path_lambda  # Small perturbation
            #print(f"DEBUG: Applied Gumbel noise (λ={self.aslr_cross_path_lambda:.3f}, τ={self.aslr_gumbel_temp:.3f})")
            
            # Cross-path collaboration (attention across N paths)
            latent_states = self.m3po_path_collaborator(latent_states, temperature=self.aslr_cross_path_temp)
            #print(f"DEBUG: Iteration {t+1}/{self.aslr_max_iterations}, shape: {latent_states.shape}")
        
        # Collapse: Aggregate N paths -> 1
        thought_state = self.m3po_path_aggregator(latent_states)  # (batch, hidden)
        #print(f"DEBUG: Collapsed to single path, shape: {thought_state.shape}")
        
        # SOFT GATING: Weighted combination (differentiable!)
        router_weight = router_weight.to(hidden_states.dtype)  # Match dtype (bfloat16)
        thought_state = thought_state.to(hidden_states.dtype)
        
        new_last_token = router_weight * thought_state + (1 - router_weight) * last_token_hidden
        
        # Reconstruct hidden_states with new last token
        if hidden_states.shape[1] > 1:
            hidden_states = torch.cat([hidden_states[:, :-1, :], new_last_token.unsqueeze(1)], dim=1)
        else:
            hidden_states = new_last_token.unsqueeze(1)
        
        #print(f"DEBUG [Router]: Soft weights min={router_weight.min().item():.3f}, max={router_weight.max().item():.3f}, mean={router_weight.mean().item():.3f}")
...
def _LlamaModel_fast_forward_inference(
    ...
    def LlamaModel_fast_forward_inference_custom(
        ...
        **kwargs,
        ...
        # M3PO/ASLR Logic for GRPO Generation (N=4, T=2, Gumbel-Softmax)
        is_m3po_inference = kwargs.get('is_m3po_inference', False)
        
        if not is_m3po_inference and hasattr(self.model, 'm3po_router'):
            # Soft gating (differentiable)
            router_gate_scores = torch.sigmoid(self.model.m3po_router.gate(X))  # (batch, 1, 1)
            router_weight = router_gate_scores[:, -1, :].squeeze(-1)  # (batch,)
            
            # Store router stats for logging (attach to model)
            if not hasattr(self.model, '_m3po_router_stats'):
                self.model._m3po_router_stats = {
                    'weights': [],
                    'activations': []
                }
            self.model._m3po_router_stats['weights'].extend(router_weight.detach().cpu().tolist())
            threshold = self.model.aslr_threshold if hasattr(self.model, 'aslr_threshold') else 0.5
            self.model._m3po_router_stats['activations'].extend((router_weight > threshold).cpu().tolist())
            
            # Get last token
            X_last = X[:, -1, :]  # (batch, hidden)
            
            # Latent Expansion: N paths
            latent_states = X_last.unsqueeze(1).repeat(1, self.model.aslr_num_paths, 1)  # (batch, N, hidden)
            
            # Latent Iterations: T refinement steps with GUMBEL-SOFTMAX
            for t in range(self.model.aslr_max_iterations):
                # GUMBEL-SOFTMAX: Inject stochastic noise
                gumbel_noise = F.gumbel_softmax(
                    torch.zeros_like(latent_states), 
                    tau=self.model.aslr_gumbel_temp, 
                    hard=False
                )
                latent_states = latent_states + gumbel_noise * self.model.aslr_cross_path_lambda
                
                # Cross-path collaboration
                latent_states = self.model.m3po_path_collaborator(
                    latent_states, 
                    temperature=self.model.aslr_cross_path_temp
                )
            
            # Collapse: N -> 1
            thought_X = self.model.m3po_path_aggregator(latent_states)  # (batch, hidden)
            
            # SOFT weighted combination (differentiable!)
            router_weight = router_weight.unsqueeze(-1).to(X.dtype)  # (batch, 1)
            thought_X = thought_X.to(X.dtype)
            new_X_last = router_weight * thought_X + (1 - router_weight) * X_last
            
            X = new_X_last.unsqueeze(1)  # (batch, 1, hidden)
            
            #print(f"DEBUG [Inference Router]: Soft weights min={router_weight.min().item():.3f}, max={router_weight.max().item():.3f}")
...
class FastLlamaModel:
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        # M3PO: Add M3PO modules to modules_to_save if model has them
        if hasattr(model, 'm3po_router'):
            if modules_to_save is None:
                modules_to_save = []
            m3po_modules = ['m3po_router', 'm3po_collaborator', 'm3po_aggregator']
            for m3po_module in m3po_modules:
                if m3po_module not in modules_to_save:
                    modules_to_save.append(m3po_module)
            print("[Unsloth+M3PO] M3PO modules will be saved during training")
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    train_embed_tokens = True
                elif module in ['m3po_router', 'm3po_collaborator', 'm3po_aggregator']:
                    # M3PO modules are allowed
                    pass
                else:
                    raise TypeError(
                        f"Unsloth: Module = {module} is not allowed. Only 'lm_head', 'embed_tokens', and M3PO modules are allowed."
                    )
    ...
    def patch_peft_model(
        ...
        # M3PO: Set M3PO components to FP32 for Gumbel-Softmax stability
        if hasattr(model, 'm3po_router'):
            model.m3po_router.to(torch.float32)
            model.m3po_collaborator.to(torch.float32)
            model.m3po_aggregator.to(torch.float32)
            print("[Unsloth+M3PO] M3PO modules set to FP32")
