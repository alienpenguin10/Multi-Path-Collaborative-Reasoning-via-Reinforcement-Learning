...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # ASLR (Adaptive Stochastic Latent Reasoning) configuration
        self.aslr_num_paths = args.aslr_num_paths if hasattr(args, 'aslr_num_paths') else None
        self.aslr_max_iterations = args.aslr_max_iterations if hasattr(args, 'aslr_max_iterations') else None
        self.aslr_threshold = args.aslr_threshold if hasattr(args, 'aslr_threshold') else None
        self.aslr_gumbel_temp = args.aslr_gumbel_temp if hasattr(args, 'aslr_gumbel_temp') else None
        self.aslr_cross_path_lambda = args.aslr_cross_path_lambda if hasattr(args, 'aslr_cross_path_lambda') else None
        self.aslr_cross_path_temp = args.aslr_cross_path_temp if hasattr(args, 'aslr_cross_path_temp') else None

        if self.aslr_num_paths is not None:
            print("[M3PO/ASLR] Adaptive Stochastic Latent Reasoning ENABLED")
            print(f"  N paths per token: {self.aslr_num_paths}")
            print(f"  T iterations per token: {self.aslr_max_iterations}")
            print(f"  Router threshold: {self.aslr_threshold}")
            print(f"  Gumbel temperature: {self.aslr_gumbel_temp}")
            print(f"  Cross-path λ: {self.aslr_cross_path_lambda}")
            print(f"  Cross-path temp: {self.aslr_cross_path_temp}")
            ...
            with (
                ...
                prompt_completion_ids = unwrapped_model.generate(
                    **generate_inputs,
                    ...
                    is_m3po_inference=False,  # Enable ASLR during GRPO generation
